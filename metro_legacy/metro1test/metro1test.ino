...
int* lignes[13] = { &ligne1, &ligne3, &ligne4, &ligne5, &ligne6, &ligne7, &ligne8, &ligne9, &ligne10, &ligne11, &ligne12, &ligne13, &ligne14 };
...
void loop(){
  for (int j=0; j<13; j++) {
  int *pointeurligne = lignes[j];
  for (int i=0; i<sizeof(pointeurligne); i++) {
    
    Wire.beginTransmission(pointeurligne->i2c); // PCF8574P GGG
    Wire.write(pointeurligne->led);
